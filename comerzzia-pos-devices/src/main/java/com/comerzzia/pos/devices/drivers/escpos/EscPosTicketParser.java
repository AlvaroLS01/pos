...
public class EscPosTicketParser extends DefaultHandler {
    
    private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(EscPosTicketParser.class.getName());
    
    private static SAXParser m_sp = null;
    
    private EscPosDeviceTicket m_printer;
    ...
    private EscPosDevicePrinter m_oOutputPrinter;
    
    
    /** Creates a new instance of TicketParser */
    public EscPosTicketParser(EscPosDeviceTicket printer) {
        m_printer = printer;
    }
    
    public void printTicket(String sIn) throws EscPosTicketPrinterException {
        printTicket(new StringReader(sIn));
    }
    
    public void printTicket(Reader in) throws EscPosTicketPrinterException  {
        ...
        try {
            ...
            log.error("printTicket() - error parseando configuration");
            throw new EscPosTicketPrinterException("error parsing configuration", ePC);
        } catch (SAXException eSAX) {
            log.error("printTicket() - error sax ");
            throw new EscPosTicketPrinterException("error sax", eSAX);
        } catch (IOException eIO) {
            log.error("printTicket() - error io");
            throw new EscPosTicketPrinterException("error io", eIO);
        } catch (Exception ex) {
            log.error("printTicket() - Error no controlado :"+ex.getMessage(), ex);
            throw new EscPosTicketPrinterException("Error no controlado", ex);
        }
    }    
    ...
    public void startDocument() throws SAXException {
        ...
        bcposition = null;
        m_sVisorLine = null;
        m_iVisorAnimation = EscPosDeviceDisplayBase.ANIMATION_NULL;
        m_sVisorLine1 = null;
        m_sVisorLine2 = null;
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException{
        ...
        case OUTPUT_NONE:
            if ("opendrawer".equals(qName)) {
                ...
                if ("scroll".equals(animation)) {
                    m_iVisorAnimation = EscPosDeviceDisplayBase.ANIMATION_SCROLL;
                } else if ("flyer".equals(animation)) {
                    m_iVisorAnimation = EscPosDeviceDisplayBase.ANIMATION_FLYER;
                } else if ("blink".equals(animation)) {
                    m_iVisorAnimation = EscPosDeviceDisplayBase.ANIMATION_BLINK;
                } else if ("curtain".equals(animation)) {
                    m_iVisorAnimation = EscPosDeviceDisplayBase.ANIMATION_CURTAIN;
                } else { // "none"
                    m_iVisorAnimation = EscPosDeviceDisplayBase.ANIMATION_NULL;
                }
                m_sVisorLine1 = null;
        ...
        case OUTPUT_TICKET:
            if ("image".equals(qName)){
                ...
                m_oOutputPrinter.beginLine(parseInt(attributes.getValue("size"), EscPosDevicePrinter.SIZE_0));
            } else if ("text".equals(qName)) {
                text = new StringBuffer();
                m_iTextStyle = ("true".equals(attributes.getValue("bold")) ? EscPosDevicePrinter.STYLE_BOLD : EscPosDevicePrinter.STYLE_PLAIN)
                             | ("true".equals(attributes.getValue("underline")) ? EscPosDevicePrinter.STYLE_UNDERLINE : EscPosDevicePrinter.STYLE_PLAIN);
                String sAlign = attributes.getValue("align");
                if ("right".equals(sAlign)) {
                    m_iTextAlign = EscPosDevicePrinter.ALIGN_RIGHT;
                } else if ("center".equals(sAlign)) {
                    m_iTextAlign = EscPosDevicePrinter.ALIGN_CENTER;
                } else {
                    m_iTextAlign = EscPosDevicePrinter.ALIGN_LEFT;
                }
...
//                    log.error("endElement() - Elemanto Play no soportado");
//                }
//                text = null;
//            } 
            break;
...
//                    }
                    ...
                    log.error("endElement() - Excepción con imagen en parseador de ticket");
                }
                text = null;
            } else if ("barcode".equals(qName)) {
                ...
                if (m_iTextLength > 0) {
                    switch(m_iTextAlign) {
                    case EscPosDevicePrinter.ALIGN_RIGHT:
                        m_oOutputPrinter.printText(m_iTextStyle, EscPosDeviceTicket.alignRight(text.toString(), m_iTextLength));
                        break;
                    case EscPosDevicePrinter.ALIGN_CENTER:
                        m_oOutputPrinter.printText(m_iTextStyle, EscPosDeviceTicket.alignCenter(text.toString(), m_iTextLength));
                        break;
                    default: // DevicePrinter.ALIGN_LEFT
                        m_oOutputPrinter.printText(m_iTextStyle, EscPosDeviceTicket.alignLeft(text.toString(), m_iTextLength));
                        break;
                    }
                } else {
        ...
        case OUTPUT_DISPLAY:
            if ("line".equals(qName)) { // line 1 or 2 of the display
                ...
                if (m_iTextLength > 0) {
                    switch(m_iTextAlign) {
                    case EscPosDevicePrinter.ALIGN_RIGHT:
                        m_sVisorLine.append(EscPosDeviceTicket.alignRight(text.toString(), m_iTextLength));
                        break;
                    case EscPosDevicePrinter.ALIGN_CENTER:
                        m_sVisorLine.append(EscPosDeviceTicket.alignCenter(text.toString(), m_iTextLength));
                        break;
                    default: // DevicePrinter.ALIGN_LEFT
                        m_sVisorLine.append(EscPosDeviceTicket.alignLeft(text.toString(), m_iTextLength));
                        break;
                    }
                } else {
