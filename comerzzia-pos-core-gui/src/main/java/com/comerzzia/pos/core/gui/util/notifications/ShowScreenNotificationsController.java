package com.comerzzia.pos.core.gui.util.notifications;
...
@Component
public class ShowScreenNotificationsController extends ModalSceneController {
	...
	public void onSceneOpen() throws InitializeGuiException {
		...
		List<ScreenNotificationDto> notifications = (List<ScreenNotificationDto>) sceneData.get(PARAM_NOTIFICATIONS);
		if(notifications == null) {
			throw new InitializeGuiException(I18N.getText("No se han especificado las notificaciones. Contacte con un administrador si el problema persiste."));
		}
		
		String title = (String) sceneData.get(PARAM_TITLE);
	...
	private void showNotifications(List<ScreenNotificationDto> notifications) {
		for(ScreenNotificationDto notification : notifications) {
			...
			lbNotificationTitle.getStyleClass().addAll("medium-text","bold");
