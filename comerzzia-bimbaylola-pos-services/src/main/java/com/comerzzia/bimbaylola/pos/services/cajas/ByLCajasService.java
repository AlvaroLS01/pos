...
public class ByLCajasService extends CajasService {
	...
	public void cerrarCaja(SqlSession sqlSession, Caja caja, Date fechaCierre)
		...
		procesarMensajeRecuentosCaja(sqlSession, caja);
		crearDocumentoCierreCaja(sqlSession, caja, fechaCierre);
		super.cerrarCaja(sqlSession, caja, fechaCierre);
	}
	...
	public void crearDocumentoCierreCaja(SqlSession sqlSession, Caja caja, Date fechaCierre) throws DocumentoException,
		...
		if (caja != null) {
	        ...
	        TimeZone zonaLocal = TimeZone.getDefault();
	        int offsetMillisApertura = zonaLocal.getOffset(caja.getFechaApertura().getTime());	        
			cierreCaja.setFechaApertura(new Date(caja.getFechaApertura().getTime()-offsetMillisApertura));
			int offsetMillisCierre = zonaLocal.getOffset(caja.getFechaCierre().getTime());	 
			cierreCaja.setFecha(new Date(caja.getFechaCierre().getTime()-offsetMillisCierre));
	...
	private void procesarMensajeRecuentosCaja(SqlSession sqlSession, Caja caja) throws TicketsServiceException, CajasServiceException {
		try {
			Tienda tienda = sesion.getAplicacion().getTienda();
			...
			ticket.setFecha(caja.getFechaCierre());
			...
			recuentoCajaBean.setFecha(caja.getFechaCierre());
			...
			List<RecuentoBean> recuentosBean = new ArrayList<RecuentoBean>();
	...
	public Integer countTurnosCaja(Caja caja) {
		if (caja != null) {
			...
			Date fechaInicio = java.sql.Timestamp.valueOf(fechaEspecifica.atStartOfDay());
			Date fechaFin = java.sql.Timestamp.valueOf(fechaEspecifica.atTime(LocalTime.MAX));

			CajaExample cajaExample = new CajaExample();
			cajaExample.or().andCodAlmacenEqualTo(caja.getCodAlm()).andCodcajaEqualTo(caja.getCodCaja()).andFechaAperturaBetween(fechaInicio, fechaFin);
