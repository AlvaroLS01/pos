...
import com.comerzzia.dinosol.pos.services.payments.methods.types.tefvirtual.TefBasicManager;
...
public class TefSipayManager extends TefBasicManager {
	...
	public boolean pay(BigDecimal amount) throws PaymentException {
		...
		try {
			...
			BigDecimal amountGateway = new BigDecimal(response.getImporte());
			if(!BigDecimalUtil.isIgual(amount, amountGateway) && !response.isDCC()) {
				log.error("pay() - El pago recibido desde la pasarela es distinto al pago solicitado.");
				log.error("pay() - Pago solicitado: " + amount);
				log.error("pay() - Pago recibido: " + amountGateway);
				cancel();
				return false;
			}
