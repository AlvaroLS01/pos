...
public class ServicioContratoMascotas {
	...
	public DatosCabeceraContrato generarContrato(IskaypetLineaTicket iskaypetLineaTicket, Map<String, Object> datosAdicionales) {
		...
		TicketContratosBean contratoInsertado = getContratosRealizadoByPrimaryKey(uidTicket, iskaypetLineaTicket.getIdLinea());
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm");
		//CZZ-2655 - La fecha se utiliza para enviarla en el asunto a Evicertia, si ya esta insertado y se reenvia la fecha no debe cambiar.
		Date fechaEnvio = contratoInsertado != null? contratoInsertado.getFecha() : new Date();
		String asunto = getAsunto(sesion.getAplicacion().getCodAlmacen(), sesion.getAplicacion().getCodCaja(), fechaEnvio, iskaypetLineaTicket.getContratoAnimal().getEmail());
		...
		datosCabecera.setFechaContrato(sdf.format(fechaEnvio));
		...
		try {
			...
			contratoMascota.setFechaEnvio(fechaEnvio);
			contratoMascota.setUidTicket(uidTicket);
			generarXMLContratoMascota(contratoMascota);
			
			iskaypetLineaTicket.getContratoAnimal().setFecha(fechaEnvio);
			//GAP 117 RECUPERACIÓN DE CONTRATOS DESDE EL POS
	...
	/**
	 * CZZ-2655 - Nuevo metodo para generar el asunto del correo que se envía a Evicertia, antes era el codTicket y ahora se tiene que generar.
	 * @param tienda
	 * @param caja
	 * @param email
	 * @return
	 */
	public static String getAsunto(String tienda, String caja, Date fechaEnvio, String email) {
	    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");

	    String fechaHora = (fechaEnvio != null) ? sdf.format(fechaEnvio) : "";

	    String safeTienda = (tienda != null) ? tienda : "";
	    String safeCaja = (caja != null) ? caja : "";
	    String safeEmail = (email != null) ? email : "";

	    return String.format("%s/%s | %s | %s", safeTienda, safeCaja, fechaHora, safeEmail);
	}
	...
	public DatosCabeceraContrato convertir(IskaypetLineaTicket linea, TicketContratosBean ticketContrato, String codLenguaje ) {
		...
		// Otros datos
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm");
		String fechaContrato = ticketContrato.getFecha()!=null && ticketContrato.getFecha()!=null? sdf.format(ticketContrato.getFecha()) : StringUtils.EMPTY;
	...
	private String completarContrato(DatosCabeceraContrato datosCabecera, String idiomaFidelizado) {
		...
		try {
			...
			if(StringUtils.isBlank(datosCabecera.getFechaContrato())) {
				datosCabecera.setFechaContrato(datosCabecera.getContratoAnimal()!=null && datosCabecera.getContratoAnimal().getFecha()!=null? sdf.format(datosCabecera.getContratoAnimal().getFecha()): "");
			}
			try {
	...
	public void generarXMLContratoMascota(ContratoMascotaDto contratoMascota) {
		...
		try {
			...
			String uid = StringUtils.isNotBlank(contratoMascota.getUidTicket())? contratoMascota.getUidTicket() : UUID.randomUUID().toString();
	...
	public void insertContratoRealizado(ContratoMascotaDto contratoMascota, Integer idLinea) {
		...
		TicketContratosBean contratoInsertado = getContratosRealizadoByPrimaryKey(contratoMascota.getUidTicket(), idLinea);
