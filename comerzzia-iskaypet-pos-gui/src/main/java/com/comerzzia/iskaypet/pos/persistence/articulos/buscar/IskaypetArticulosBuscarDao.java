...
public class IskaypetArticulosBuscarDao extends ArticulosBuscarDao {
    ...
    public static List<ArticuloBuscarBean> buscarArticulosIskaypet(Connection conn, ArticulosParamBuscar param, Sesion sesion) throws SQLException {
		...
		sql =
			...
			"WHERE 1=1 AND " +
				"art.UID_ACTIVIDAD = ? " +
                    matchCodArt(param.getCodigo()) +
                    matchDesArt(param.getDescripcion()) +
				whereActivo +
			"ORDER BY " +
				"CODART ";
        ...
        try {
            pstmt = new PreparedStatement(conn, sql);
            int idx = 1;
            pstmt.setString(idx++, param.getUidActividad());
            if(StringUtils.isNotBlank(param.getCodigo())){
                pstmt.setString(idx++, param.getCodigo().trim().toLowerCase() + "*");
            }
            if (StringUtils.isNotBlank(param.getDescripcion())) {
                String transformed = Arrays.stream(param.getDescripcion().trim().split("\\s+"))
                        .map(word -> "+" + word + "*")
                        .collect(Collectors.joining(" "));
                pstmt.setString(idx++, transformed);
            }
    ...
    private static String matchCodArt(String codArt){
        if(StringUtils.isNotBlank(codArt)){
            return "AND MATCH (art.CODART) AGAINST (? IN BOOLEAN MODE) ";
        }
        return "";
    }

    private static String matchDesArt(String desArt){
        if(StringUtils.isNotBlank(desArt)){
            return "AND MATCH (art.DESART) AGAINST (? IN BOOLEAN MODE) ";
        }
        return "";
    }
}
