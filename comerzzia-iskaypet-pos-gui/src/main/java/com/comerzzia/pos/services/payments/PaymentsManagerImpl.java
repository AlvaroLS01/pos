...
public class PaymentsManagerImpl implements PaymentsManager {
	...
	private Logger log = Logger.getLogger(PaymentsManagerImpl.class);
	...
	@Override
	public void setPaymentsMethodsConfiguration(PaymentsMethodsConfiguration configuration) {
		for (PaymentMethodConfiguration paymentMethodConfiguration : configuration.getPaymentsMethodsConfiguration()) {
			try {
	...
	protected void recordCashFlow(String paymentCode, BigDecimal amount, boolean isIncome) throws CajasServiceException {
	    ...
	    movimiento.setCodMedioPago(paymentCode);
