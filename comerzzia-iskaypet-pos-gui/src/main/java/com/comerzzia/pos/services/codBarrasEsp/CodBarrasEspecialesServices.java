...
import com.comerzzia.pos.persistence.codBarras.CodigoBarrasExample;
import com.comerzzia.pos.persistence.codBarras.CodigoBarrasKey;
import com.comerzzia.pos.persistence.codBarras.CodigoBarrasMapper;
 ...
 */
@Service
public class CodBarrasEspecialesServices {
	
	protected static final Logger log = Logger.getLogger(CodBarrasEspecialesServices.class);
	
	public static List<CodigoBarrasBean> codigos;
	
	@Autowired
	protected Sesion sesion;
	
	@Autowired
	protected CodigoBarrasMapper codigoBarrasMapper;
	
	public void cargarCodigosBarrasEspeciales(String uidActividad) throws CodBarrasEspecialesException{
		
		try{
			codigos = obtenerCodigosBarras(uidActividad);
		}
		catch (Exception e) {
			String msg = "Se ha producido un error consultando los codigos de barras: " + e.getMessage();
			log.error("cargarCodigosBarrasEspeciales() - " + msg, e);
			throw new CodBarrasEspecialesException(I18N.getTexto("Error cargando los códigos de barras especiales"));
		}
	}
	
	public List<CodigoBarrasBean> obtenerCodigosBarras(String uidActividad) throws Exception{
		log.trace("obtenerCodigosBarras() -");
		SqlSession sqlSession = new SqlSession();

        try {
            sqlSession.openSession(SessionFactory.openSession());
            CodigoBarrasExample example = new CodigoBarrasExample();
            CodigoBarrasExample.Criteria criteria = example.createCriteria();
            ...
            String msg = "Se ha producido un error consultando los codigos de barras: " + e.getMessage();
            log.error("obtenerCodigosBarras() - " + msg, e);
            throw e;
        }
        finally {
            sqlSession.close();
	...
	public int eliminarCodigo(CodigoBarrasBean codBar) throws Exception{
		log.trace("eliminarCodigo() - consultando codigos de barras");
		SqlSession sqlSession = new SqlSession();

        try {
            sqlSession.openSession(SessionFactory.openSession());
            CodigoBarrasKey key = new CodigoBarrasKey();
            key.setOrden(codBar.getOrden());
            key.setUidActividad(sesion.getAplicacion().getUidActividad());
            
            int res = codigoBarrasMapper.deleteByPrimaryKey(key);
            
            return res;
        }
        catch (Exception e) {
            String msg = "Se ha producido un error eliminando el codigo de barras: " +codBar.getOrden() + " :" + e.getMessage();
            log.error("eliminarCodigo() - " + msg, e);
            throw e;
        }
        finally {
            sqlSession.close();
	...
	public int nuevoCodigo(CodigoBarrasBean codBar) throws Exception{
        ...
        try {
            ...
            int res =  codigoBarrasMapper.insert(codBar);
            
            return res;
        }
        catch (Exception e) {
            String msg = "Se ha producido un error creando un nuevo codigo de barras: " + e.getMessage();
            log.error("nuevoCodigo() - " + msg, e);
            throw e;
        }
        finally {
            sqlSession.close();
	...
	public int actualizarCodigo(CodigoBarrasBean codBar) throws Exception{
		log.trace("actualizarCodigo() - consultando codigos de barras");
		SqlSession sqlSession = new SqlSession();

        try {
            sqlSession.openSession(SessionFactory.openSession());
            
            int res = codigoBarrasMapper.updateByPrimaryKey(codBar);
            
            return res;
        }
        catch (Exception e) {
            String msg = "Se ha producido un error actualizando el código de barras: " + codBar.getOrden() + " :" + e.getMessage();
            log.error("actualizarCodigo() - " + msg, e);
            throw e;
        }
        finally {
            sqlSession.close();
	...
	public int obtenerSiguienteOrdenDisponible() throws Exception{
		log.trace("obtenerSiguienteOrdenDisponible() - ");
		
		String uidActividad = sesion.getAplicacion().getUidActividad();
        SqlSession sqlSession = new SqlSession();
        try {
        	sqlSession.openSession(SessionFactory.openSession());
            CodigoBarrasExample exampleCodBarras = new CodigoBarrasExample();
            exampleCodBarras.or().andUidActividadEqualTo(uidActividad);
            Integer linea = codigoBarrasMapper.selectUltimoOrden(exampleCodBarras);
            if (linea == null) {
                linea = 1;
            }
            return linea;
        	...
        	String msg = "Se ha producido un error obteniendo el siguiente orden disponible: " + e.getMessage();
            log.error("obtenerSiguienteOrdenDisponible() - " + msg, e);
            throw e;
        }
        finally {
            sqlSession.close();
