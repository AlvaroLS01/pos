...
import com.comerzzia.api.rest.client.response.ResponsePutRest;
import com.comerzzia.api.rest.client.tickets.TicketRequestRest;
...
public class TicketsService {
     ...
     */
    protected synchronized void procesarTicket(final TicketBean ticket, final byte[] xmlTicket) throws Exception {
    	Thread hiloEnvio = new Thread(new Runnable(){
			...
			protected void marcarTicketProcesado(TicketBean ticket) {
				...
				if(!idTipoTienda.equals(1L)) {
					...
					try {
						...
						ticketMapper.updateByPrimaryKey(ticketBBDD);
			...
			protected boolean enviarTicket(final TicketBean ticket, final byte[] xmlTicket) throws UnsupportedEncodingException, RestException, RestHttpException {
    ...
    public List<TicketBean> consultarTicketLocalizador(String localizador, List<Long> idTiposDocValidos) throws TicketsServiceException{
        ...
        try {
            ...
            ticketLocatorKey.setUidActividad(sesion.getAplicacion().getUidActividad());
    ...
    public TicketBean consultarTicketAbono(String codTienda, String codCaja, String codTicket, Long idTipoDoc) throws TicketsServiceException{
        ...
        try {
            ...
            TicketExample example = new TicketExample();
            TicketExample.Criteria criteria = example.createCriteria();
            criteria.andUidActividadEqualTo(sesion.getAplicacion().getUidActividad()).andCodAlmacenEqualTo(codTienda).andIdTicketEqualTo(Long.parseLong(codTicket)).andCodcajaEqualTo(codCaja).andIdTipoDocumentoEqualTo(idTipoDoc);
    ...
    protected String generarFirma(SqlSession sqlSession, Ticket ticket){
    	...
    	try {
    		TicketBean ultimoTicket = consultarUltimoTicketFirma(sqlSession, ticket.getUidActividad(), ticket.getIdTicket(), ticket.getCabecera().getTipoDocumento(), ticket.getCabecera().getSerieTicket());
    ...
    public TicketBean consultarUltimoTicketFirma(SqlSession sqlSession, String uidActividad, Long idTicket, Long idTipoDoc, String series) throws TicketsServiceException {
        TicketBean res = null;
        try {
