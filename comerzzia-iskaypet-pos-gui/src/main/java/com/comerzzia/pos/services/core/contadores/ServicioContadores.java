		...
		if(contador.getUltimaPeticion() == null || (fechaActual.equals(contador.getUltimaPeticion()) || DateUtils.addHours(fechaActual, 1).after(contador.getUltimaPeticion()))) {
			...
			while(lock && numIntentos > 0) {
				...
				if(rango==null || (rango.getRangoFechaInicio().before(new Date()) && rango.getRangoFechaFin().after(new Date())) ){
					...
					//Esto elimina la diferencia entre distintas bases de datos a la hora de redondear los milisegundos.
					...
					filtroUpdate.or().andUidActividadEqualTo(contador.getUidActividad())
									.andIdContadorEqualTo(contador.getIdContador())
									.andDivisor1EqualTo(contador.getDivisor1())
									.andDivisor2EqualTo(contador.getDivisor2())
									.andDivisor3EqualTo(contador.getDivisor3())
									.andValorEqualTo(valorActual);
									
					// Si no se ha actualizado ningún registro, el lock seguirá a verdadero y se consultará de nuevo el valor del contador.
					lock = contadorMapper.updateByExample(contador, filtroUpdate) != 1;
					if(lock) {
						numIntentos--;
						if(numIntentos < 80) {
							log.warn(String.format("incrementarContador() - No se ha actualizado correctamente el contador [%s] (divisor1: %s, divisor2: %s, divisor3: %s) cuyo valor actual es [%s] con el nuevo valor [%s] ; se reintenta de nuevo (intentos restantes: %s)", 
										contador.getIdContador(), 
										contador.getDivisor1(),
										contador.getDivisor2(),
										contador.getDivisor3(),
										valorActual, 
										contador.getValor(), 
										numIntentos));
						}
			...
			if(numIntentos == 0) {
				String mensaje = String.format("No se ha podido actualizar el contador [%s] (divisor1: %s, divisor2: %s, divisor3: %s); se ha superado el número de intentos para obtener un valor ", 
						contador.getIdContador(),
						contador.getDivisor1(),
						contador.getDivisor2(),
						contador.getDivisor3());
