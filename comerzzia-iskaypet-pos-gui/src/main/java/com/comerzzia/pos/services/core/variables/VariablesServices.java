...
import com.comerzzia.pos.persistence.core.variables.config.POSConfigVariableMapper;
...
public class VariablesServices {
    ...
    @Autowired
    protected POSConfigVariableMapper configVariableMapper;

    /** Carga en sesión todas las variables registradas en el sistema
     * @param uidActividad
     * @throws VariablesServiceException 
     */
    public void cargarVariables(String uidActividad) throws VariablesServiceException{
        ...
        try{
            ...
            exampleVariable.or().andUidActividadEqualTo(uidActividad);
            List<ConfigVariableBean> configVariablesBean = configVariableMapper.selectByExample(exampleConfigVariable);
             
            List<VariableBean> variablesBean = variableMapper.selectByExample(exampleVariable);
            
             // Primero cargamos todos los valores por defecto de todas las variables
            for (ConfigVariableBean variable : configVariablesBean) {
                variables.put(variable.getIdVariable(), variable.getValorDefecto());
            }

            // Luego cargamos todos los valores de variables modificados
            
            for (VariableBean variable : variablesBean) {
                if(variable.getValor() != null && !variable.getValor().isEmpty()){
                    variables.put(variable.getIdVariable(), variable.getValor());
                }
                else if(variable.getValorDefecto() != null){
                    variables.put(variable.getIdVariable(), variable.getValorDefecto());
                }
                else{
                    variables.put(variable.getIdVariable(), "");
                }
            }
        }
        catch(Exception e){
            String msg = "Se ha producido un error consultando variables en el sistema para uidActividad " + uidActividad + ": " + e.getMessage();
            log.error("cargarVariables() - "+ msg);
    ...
    public BigDecimal getVariableAsBigDecimal(String idVariable) {
        String valor = variables.get(idVariable);
        if (valor != null){
    ...
    public Boolean getVariableAsBoolean(String idVariable) {
        String valor = variables.get(idVariable);
        return valor != null && !valor.equals("N");
    }
