...
import com.comerzzia.pos.persistence.core.usuarios.UsuarioKey;
...
public class UsuariosService {
    ...
    public UsuarioBean login(String usuario, String password) throws UsuarioInvalidLoginException, UsuariosServiceException {
        try {
            ...
            UsuarioBean usuarioBean = consultarUsuario(usuario);
            password = CriptoUtil.cifrar(CriptoUtil.ALGORITMO_MD5, password.getBytes());
            if (!password.equals(usuarioBean.getClave())) {
                log.debug("login() - Password incorrecto. ");
                throw new UsuarioInvalidLoginException();
            }
            return usuarioBean;
        }
        catch (UsuarioInvalidLoginException | UsuariosServiceException e) {
            throw e;
        }
     ...
     * @throws com.comerzzia.pos.services.core.usuarios.UsuariosServiceException
     */
    public UsuarioBean consultarUsuario(String usuario) throws UsuarioNotFoundException, UsuariosServiceException {
    ...
    /**
     * Realiza un cambio de password del usuario con el id indicado. Previamente comprueba
     * que el usuario tiene el password original indicado. En caso contrario, lanza una excepción.
     *
     * @param idUsuario :: Identificador del usuario
     * @param password :: Password original
     * @param nuevaPassword :: Nueva password
     * @throws UsuarioInvalidLoginException :: Lanzada si la password original es incorrecta.
     * @throws com.comerzzia.pos.services.core.usuarios.UsuariosServiceException
     */
    public void cambiarPassword(Long idUsuario, String password, String nuevaPassword) throws UsuarioInvalidLoginException, UsuariosServiceException {
        ...
        try {
            sqlSession.openSession(SessionFactory.openSession());
            String uidInstancia = sesion.getAplicacion().getUidInstancia();
            UsuarioKey keyUsuario = new UsuarioKey();
            keyUsuario.setIdUsuario(idUsuario);
            keyUsuario.setUidInstancia(uidInstancia);
            log.debug("cambiarPassword() - consultando usuario para cambiar de clave");
            UsuarioBean usuarioBean = usuarioMapper.selectByPrimaryKey(keyUsuario);

            password = CriptoUtil.cifrar(CriptoUtil.ALGORITMO_MD5, password.getBytes());

            if (usuarioBean != null) {

                nuevaPassword = CriptoUtil.cifrar(CriptoUtil.ALGORITMO_MD5, nuevaPassword.getBytes());

                UsuarioBean usuarioCambioClave = new UsuarioBean();
                usuarioCambioClave.setUidInstancia(usuarioBean.getUidInstancia());
                usuarioCambioClave.setIdUsuario(usuarioBean.getIdUsuario());
                usuarioCambioClave.setClave(nuevaPassword);

                log.debug("cambiarPassword() - actualizando clave para el usuario " + usuarioBean.getIdUsuario());
            ...
            sqlSession.rollback();
            log.error("cambiarPassword() - Cambio de contraseña incorrecto para el usuario " + idUsuario + "  y contraseña " + password);
            throw new UsuarioInvalidLoginException();
        }
        catch (Exception e) {
            sqlSession.rollback();
            String msg = "Se ha producido un error cambiando clave a usuario con id" + idUsuario + " y clave " + password + " : " + e.getMessage();
            log.error("cambiarPassword() - " + msg);
            throw new UsuariosServiceException("error.service.core.usuarios.service.cambiarPassword", e);
        }
        finally {
            sqlSession.close();
        }
	...
	protected List<UsuarioBean> filtrarUsuariosTienda(List<UsuarioBean> usuarios) {
	    ...
	    exampleAlmacenes.or().andUidActividadEqualTo(sesion.getAplicacion().getUidActividad()).andUidInstanciaEqualTo(sesion.getAplicacion().getUidInstancia())
