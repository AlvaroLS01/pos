...
public class PromocionDescuentoLineaBean extends PromocionLinea{
    ...
    public void leerDatosPromocion(byte[] datosPromocion) {
        try {
            ...
            String precioDescuentoFormat = xmlPromocion.getNodo("precioDescuento").getValue();
            setPrecioPorcentaje(new BigDecimal(precioDescuentoFormat.replace(",", ".")));
            
            String cantidadTipo = xmlPromocion.getNodo("cantidadTipo").getValue();
            if(cantidadTipo.isEmpty() || cantidadTipo == null) {
            	   setCantidad(BigDecimal.ZERO);
            } else {
            	   setCantidad(new BigDecimal(cantidadTipo.replace(",", ".")));
            }
