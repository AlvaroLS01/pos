...
import com.comerzzia.bimbaylola.pos.dispositivo.tarjeta.adyen.cloud.ByLTefAdyenCloud;
...
@Primary
@Scope("prototype")
public class ByLTicketManager extends TicketManager implements ByLTicketRegalo{
	...
	@Autowired
	protected ByLSesionImpuestos bylSesionImpuestos;
	@Autowired
	...
	private static final String COD_SINGAPUR = "SG";
	private static final String COD_ECUADOR = "EC";
	private static final String COD_COLOMBIA = "CO";
	...
	private ByLTicketManager ticketAuxiliar;
		...
		protected void succeeded() {
			...
			for (ListIterator<IPagoTicket> iterator = pagosTarjeta.listIterator(); iterator.hasNext();) {
				...
				if (Dispositivos.getInstance().getTarjeta() instanceof TefConexFlow) {
					...
					ConexFlowDatosPeticionPagoTarjeta datoPeticion = new ConexFlowDatosPeticionPagoTarjeta(((ByLCabeceraTicket) ticketPrincipal.getCabecera()).getIdTransaccion(), codTicketConexFlow,
					        pagoTicket.getImporte(), ticketPrincipal.getCabecera().getCodTipoDocumento());
						...
						for (IPagoTicket pagoTicketOrigen : pagosTarjetaOrig) {
							if (pagoTicket.getCodMedioPago().equals(pagoTicketOrigen.getCodMedioPago()) && BigDecimalUtil.isIgual(pagoTicket.getImporte().abs(), pagoTicketOrigen.getImporte().abs())) {
								...
								if (datosRespuestaPagoTarjeta != null) {
									...
									// Parche para pasar los datos necesarios para devoluciones referenciadas en
									// ConexFlow
					...
					AxisDatosPeticionPagoTarjeta datoPeticion = new AxisDatosPeticionPagoTarjeta(((ByLCabeceraTicket) ticketPrincipal.getCabecera()).getIdTransaccion(), codTicket,
					        pagoTicket.getImporte(), ticketPrincipal.getCabecera().getCodTipoDocumento());
					if (getIntentosDevolucionFallido() == 1) {
						datoPeticion.setEsDevolucionFallida(true);
					}
					...
					AdyenDatosPeticionPagoTarjeta datoPeticion;
					if (pagoTicket.getDesMedioPago().equals("ADYEN MSI")) {
						datoPeticion = new AdyenDatosPeticionPagoTarjeta(((ByLCabeceraTicket) ticketPrincipal.getCabecera()).getIdTransaccion(), codTicket, pagoTicket.getImporte(),
						        ticketPrincipal.getCabecera().getCodTipoDocumento(), ((ByLCabeceraTicket) ticketPrincipal.getCabecera()).getTotalMeses());
					}
					else {
						datoPeticion = new AdyenDatosPeticionPagoTarjeta(((ByLCabeceraTicket) ticketPrincipal.getCabecera()).getIdTransaccion(), codTicket, pagoTicket.getImporte(),
					...
					if (ticketOrigen != null) {
						...
						/*
						 * Debido al GAP1846, SÓLO se hara una devolución referenciada cuando el documento origen tenga
						 * UN SÓLO método de pago --> ADYEN En el caso de que el documento origen se haya pagado con más
						 * de un método de pago, se realizará una devolución no referenciada
						 */
			...
			TarjetaCallback<List<DatosRespuestaPagoTarjeta>> dispositivoCallback = new TarjetaCallback<List<DatosRespuestaPagoTarjeta>>(){
				...
				public void onSuccess(List<DatosRespuestaPagoTarjeta> datosRespuesta) {
					...
					addMerchantAccountToTicketPayments(ticketPrincipal);
			...
			boolean totalMayorACero;

			if (getTicket().getTienda().getCliente().getCodpais().equals(ByLPagosController.CODIGO_PAIS_PT)) {
				totalMayorACero = BigDecimalUtil.isMayorOrIgualACero(totalPagos) && !documentoActivo.getCodtipodocumento().equals(ByLPagosController.COD_TIPO_DOC_NC);
			}
			else {
				totalMayorACero = BigDecimalUtil.isMayorOrIgualACero(totalPagos);
			}

			boolean venta = totalMayorACero;

			log.debug("ByLSalvarTicketSeguridadTask::succeeded() - Total de pagos: " + totalPagos);
			log.debug("ByLSalvarTicketSeguridadTask::succeeded() - Venta: " + venta);
			...
			if (venta) {
				log.debug("ByLSalvarTicketSeguridadTask::succeeded() - Solicitando pagos de tarjeta (pago)");
				Dispositivos.getInstance().getTarjeta().solicitarPagosTarjeta(datosPeticionTarjeta, stage, dispositivoCallback);
			}
			else {
				log.debug("ByLSalvarTicketSeguridadTask::succeeded() - Solicitando pagos de tarjeta (devolución)");
	...
	@SuppressWarnings("unchecked")
	public TicketDevolucionResponse recuperarTicketDevolucionByL(String codigo, String codAlmacen, String codCaja, Long idTipoDoc) throws ArticuloNoAptoException, DocumentoException {
		...
		try {
			...
			if (localizadorDocumento != null) {
				...
				if (xmlTicketOrigen != null) {
					...
					if (!tickets.isEmpty()) {
						...
						respuesta.setListadoArticulosNoDevolver(noAptoService.consultarNoAptosLista(listadoTicketNoAptos, ticketOrigen.getUidActividad()));
					}
					else {
						throw new TicketsServiceException("No se ha encontrado ticket con el localizador: " + localizadorDocumento.getLocalizador());
					}
				}
			}

			// Si no tenemos ticket, consultamos como id de documento en lugar de como
			// localizador
			if (xmlTicketOrigen == null) {
			...
			/*
			 * Si estamos en Singapur o en Ecuador, el grupo de impuestos de la devolución será el mismo que el de la venta, y si
			 * viene nulo es una devolución de Navision y buscamos por la fecha
			 */
			if (sesion.getAplicacion().getTienda().getCliente().getCodpais().equals(COD_SINGAPUR) || sesion.getAplicacion().getTienda().getCliente().getCodpais().equals(COD_ECUADOR)) {
				if (ticketPrincipal.getCabecera().getCliente().getIdGrupoImpuestos() != null) {
					log.debug("recuperarTicketDevolucionByL() - Id Grupo Impuesto ANTES: " + ticketPrincipal.getCabecera().getCliente().getIdGrupoImpuestos());
					ticketPrincipal.getCabecera().getCliente().setIdGrupoImpuestos(ticketOrigen.getCabecera().getCliente().getIdGrupoImpuestos());
				}
				else {
					int idGrupoImpuesto = grupoImpuestosService.consultarGrupoImpuestosFecha(ticketOrigen.getCabecera().getFecha()).getIdGrupoImpuestos();
					ticketPrincipal.getCabecera().getCliente().setIdGrupoImpuestos(idGrupoImpuesto);
				}
				log.debug("recuperarTicketDevolucionByL() - Id Grupo Impuesto DESPUES: " + ticketPrincipal.getCabecera().getCliente().getIdGrupoImpuestos());
			}
			
			/*
			 * Se añade control para que no se pueda seguir la devolución en el caso de que se haya recuperado una
			 * devolución
			 */

			TipoDocumentoBean documentoTicketRecuperado = documentos.getDocumento(ticketOrigen.getCabecera().getCodTipoDocumento());
			if (!documentoTicketRecuperado.getCodtipodocumento().equals("TR") && documentoTicketRecuperado.getTiposDocumentosOrigen() != null
			        && (documentoTicketRecuperado.getTiposDocumentosOrigen().contains(Documentos.FACTURA_SIMPLIFICADA)
			                || documentoTicketRecuperado.getTiposDocumentosOrigen().contains(ByLDocumentos.BOLETA))) {
				log.warn("recuperarTicketDevolucionByL() - Se está intentando realizar una devolución de una devolución.");
				throw new TicketsServiceException("No se puede realizar devoluciones de una devolución.");
			}
		...
		catch (GrupoImpuestosServiceException e) {
			log.error("recuperarTicketDevolucion() - " + e.getClass().getName() + " - " + e.getLocalizedMessage(), e);
			return null;
		}
	...
	protected TicketDevolucionResponse obtenerTicketDevolucionCentralLocalizadorByL(String localizador, boolean throwRestExceptions) throws LineaTicketException {
		...
		TicketLocalizadorRequestRest request = new TicketLocalizadorRequestRest(sesion.getAplicacion().getUidActividad(), variablesServices.getVariableAsString(VariablesServices.WEBSERVICES_APIKEY),
		        localizador);
		try {
	...
	protected TicketDevolucionResponse obtenerTicketDevolucionCentralByL(String codCaja, String codTienda, String codOperacion, Long idTipoDocumento) throws RestHttpException, RestException {
		ConsultarTicketRequestRest request = new ConsultarTicketRequestRest(sesion.getAplicacion().getUidActividad(), variablesServices.getVariableAsString(VariablesServices.WEBSERVICES_APIKEY),
				sesion.getAplicacion().getCodAlmacen(), codOperacion, codCaja, idTipoDocumento);

		TicketDevolucionResponse ticketDevolucion;
		ticketDevolucion = ByLTicketsRest.getTicket(request, codTienda);

		return ticketDevolucion;
	}

	protected TicketDevolucionResponse obtenerTicketDevolucionCentralByLbyCodTicket(String codCaja, String codTienda, String codOperacion, Long idTipoDocumento)
	        throws RestHttpException, RestException {
		ConsultarTicketRequestRest request = new ConsultarTicketRequestRest(sesion.getAplicacion().getUidActividad(), variablesServices.getVariableAsString(VariablesServices.WEBSERVICES_APIKEY),
		        codTienda, codOperacion, codCaja, idTipoDocumento);

		TicketDevolucionResponse ticketDevolucion;
		ticketDevolucion = ByLTicketsRest.getTicketCodTicket(request, sesion.getAplicacion().getCodAlmacen(), idTipoDocumento.toString(), codTienda);
	...
	public void llamarPinpad(final Stage stage, final SalvarTicketCallback callback) {
		...
		for (ListIterator<IPagoTicket> iterator = pagosTarjeta.listIterator(); iterator.hasNext();) {
			...
			if (Dispositivos.getInstance().getTarjeta() instanceof TefConexFlow) {
				...
				/* Cargamos el código del Ticket y creaamos el objeto de datos peticion */
				String codTicket = crearCodTicketPagoTarjeta(ticketPrincipal.getCabecera().getCodTicket());
				AxisDatosPeticionPagoTarjeta datoPeticion = new AxisDatosPeticionPagoTarjeta(((ByLCabeceraTicket) ticketPrincipal.getCabecera()).getIdTransaccion(), codTicket, pagoTicket.getImporte(),
				        ticketPrincipal.getCabecera().getCodTipoDocumento());
				datosPeticionTarjeta.add(datoPeticion);
		...
		TarjetaCallback<List<DatosRespuestaPagoTarjeta>> dispositivoCallback = new TarjetaCallback<List<DatosRespuestaPagoTarjeta>>(){
			...
			public void onSuccess(List<DatosRespuestaPagoTarjeta> datosRespuesta) {
				...
				addMerchantAccountToTicketPayments(ticketPrincipal); // BYL-332
				crearClaseRegistrarTicketTask(stage, callback, datosRespuesta).start();
			}
			...
			public void onFailure(List<DatosRespuestaPagoTarjeta> datosRespuesta, Throwable caught) {
				...
				/* Para comprobar los intentos fallidos para devolver */
				if (Dispositivos.getInstance().getTarjeta() instanceof TefConexFlow) {
					if (((ConexFlowDatosPeticionPagoTarjeta) datosRespuesta) != null && ((ConexFlowDatosPeticionPagoTarjeta) datosRespuesta.get(0).getDatosPeticion()) != null
					        && ((ConexFlowDatosPeticionPagoTarjeta) datosRespuesta.get(0).getDatosPeticion()).getEsDevolucion()) {
					...
					if (((AxisDatosPeticionPagoTarjeta) datosRespuesta) != null && ((AxisDatosPeticionPagoTarjeta) datosRespuesta.get(0).getDatosPeticion()) != null
					        && ((AxisDatosPeticionPagoTarjeta) datosRespuesta.get(0).getDatosPeticion()).getEsDevolucion()) {
	...
	protected void procesarTarjetaRegalo(Stage stage) throws TarjetaRegaloException, DocumentoException {
		try {
			...
			if (tarjRegalo != null) {
				...
				/**/
				/*
				 * Si es una operacion de carga inicial en tarjeta mandamos a recargar la tarjeta con el importe
				 * definido
				 */
				if (esRecargaTarjetaRegalo) {
					...
					((ByLGiftCard) Dispositivos.getInstance().getGiftCard()).recargarImporteTarjetaRegalo(
					        ...
					        // tarjRegalo.getSaldo(),
					        tarjRegalo.getImporteRecarga(), tarjRegalo.getNumTarjetaRegalo(),
					        // ticketPrincipal.getUidTicket(),
					        // ticketPrincipal.getCabecera().getDesTipoDocumento() + " " +
					        ticketPrincipal.getCabecera().getCodTicket(), ((ByLCabeceraTicket) ticketPrincipal.getCabecera()).getTarjeta().getEstado());
					...
					try {
						...
						if (giftCard.getEstado() == 1 && BigDecimalUtil.isIgual(tarjRegalo.getSaldo(), total)) {
							((ByLGiftCard) Dispositivos.getInstance().getGiftCard()).recargarImporteTarjetaRegalo(
							        ...
							        // tarjRegalo.getSaldoProvisional(),
							        // tarjRegalo.getSaldo(),
							        total.negate(), tarjRegalo.getNumTarjetaRegalo(),
							        // ticketPrincipal.getUidTicket(),
							        // ticketPrincipal.getCabecera().getDesTipoDocumento() + " " +
							        ticketPrincipal.getCabecera().getCodTicket(), ((ByLCabeceraTicket) ticketPrincipal.getCabecera()).getTarjeta().getEstado());
				...
				((ByLGiftCard) Dispositivos.getInstance().getGiftCard()).crearMovimientosProvisionales(stage, sesion.getAplicacion().getUidActividad(), listaPagosGiftcard,
				...
				// }else{
				// codTicket = ticketPrincipal.getCabecera().getDesTipoDocumento() + " " +
	...
	protected void anularMovimientoTarjetaRegalo(Stage stage) {
		...
		if (!Dispositivos.getInstance().getGiftCard().getClass().equals(GiftCardNoConfig.class)) {
			...
			if (tarjRegalo != null) {
				...
				List<String> listaTarjetasRegaloAnulables = SpringContext.getBean(ByLGiftCard.class).obtenerTarjetasRegaloAnulables();
				...
				if (pagosTarjetaRegalo != null && !pagosTarjetaRegalo.isEmpty()) {
					for (PagoTicket pago : pagosTarjetaRegalo) {
						for (GiftCardBean giftCardBean : pago.getGiftcards()) {
							if (listaTarjetasRegaloAnulables != null && !listaTarjetasRegaloAnulables.isEmpty()) {
								for (String tarjetaRegaloAnulable : listaTarjetasRegaloAnulables) {
									if(giftCardBean.getNumTarjetaRegalo().equals(tarjetaRegaloAnulable)) {
		...
		protected void succeeded() {
			...
			for (PagoTicket pago : pagosTarjeta) {
				...
				if (Dispositivos.getInstance().getTarjeta() instanceof TefConexFlow) {
					...
					/* ----------------- INICIO ----------------------------- */
					/*
					 * ----------------- Para genera el codTicket de reserva y poderlo enviar como merchantReference a
					 * Adyen -----------------------------
					 */
					if ("RE".equals(ticketPrincipal.getCabecera().getCodTipoDocumento())) {
		...
		for (DatosRespuestaPagoTarjeta datoRespuesta : datosRespuesta) {
			if (datoRespuesta != null) {
				if (Dispositivos.getInstance().getTarjeta() instanceof TefConexFlow) {
					...
					ConexFlowDatosPeticionPagoTarjeta datoPeticion = new ConexFlowDatosPeticionPagoTarjeta(datosPeticionOrig.getIdTransaccion(), datosPeticionOrig.getIdDocumento(),
				...
				// init Incidencia 106269
					...
					AdyenDatosPeticionPagoTarjeta datosPeticionOrig = (AdyenDatosPeticionPagoTarjeta) datoRespuesta.getDatosPeticion();
					AdyenDatosPeticionPagoTarjeta datoPeticion = new AdyenDatosPeticionPagoTarjeta(datosPeticionOrig.getIdTransaccion(), datosPeticionOrig.getIdDocumento(),
					        datosPeticionOrig.getImporte(), datosPeticionOrig.getCodTipoDocumento());

					datoPeticion.setCodAutorizacion(datoRespuesta.getCodAutorizacion());
					datoPeticion.setNumOpBanco(datoRespuesta.getNumOperacionBanco());
					datoPeticion.setNumOperacion(datoRespuesta.getNumOperacion());
					datoPeticion.setNumTransaccion(datoRespuesta.getNumTransaccion()); // Contiene PSP Reference,
					                                                                   // necesario para cancelación
					                                                                   // del pago.
					datosPeticionTarjeta.add(datoPeticion);
				}
	...
	protected void confirmarPagosTarjeta(List<DatosRespuestaPagoTarjeta> datosRespuesta, final Stage stage) {
		...
		for (DatosRespuestaPagoTarjeta datoRespuesta : datosRespuesta) {
			if (Dispositivos.getInstance().getTarjeta() instanceof TefConexFlow) {
				...
				AxisDatosPeticionPagoTarjeta datosPeticionOrig = (AxisDatosPeticionPagoTarjeta) datoRespuesta.getDatosPeticion();
				AxisDatosPeticionPagoTarjeta datoPeticion = new AxisDatosPeticionPagoTarjeta(datosPeticionOrig.getIdTransaccion(), datosPeticionOrig.getIdDocumento(), datosPeticionOrig.getImporte(),
				        datosPeticionOrig.getCodTipoDocumento());
	...
	protected void descontarLineasNegativasTicketOrigen() {
		...
		while (it.hasNext()) {
			...
			if (BigDecimalUtil.isMenorACero(linea.getImporteTotalConDto()) || linea.getDocumentoOrigen() != null) {
		...
		for (LineaTicket lineaNegativa : lineasNegativas) {
			...
			for (LineaTicket linea : (List<LineaTicket>) ticketOrigen.getLineas()) {
				if (linea.getCodArticulo().equals(lineaNegativa.getCodArticulo()) && linea.getDesglose1().equals(lineaNegativa.getDesglose1())
				        && linea.getDesglose2().equals(lineaNegativa.getDesglose2()) && BigDecimalUtil.isIgual(linea.getImporteTotalConDto(), lineaNegativa.getImporteTotalConDto().abs())) {
	...
	protected void asignarLineasDevueltas(ResponseGetTicketDev res) {
		...
		if (sesion.getAplicacion().getTienda().getCliente().getCodpais().equals(COD_SINGAPUR) || sesion.getAplicacion().getTienda().getCliente().getCodpais().equals(COD_ECUADOR)) {
			try {
				bylSesionImpuestos.init(ticketOrigen.getCabecera().getCliente().getIdGrupoImpuestos() != null ? ticketOrigen.getCabecera().getCliente().getIdGrupoImpuestos() : 10016);
			}
			catch (SesionInitException e) {
				log.error("asignarLineasDevueltas() - Error inicializando la sesion de impuestos: " + e.getMessage());
			}
		}
		...
		// calculamos el precio origen restando el precio de las promociones de menos
		// ingreso
		for (LineaTicketAbstract lineaOrigen : lineas) {
			...
			if (promocionMenosIngreso && !BigDecimalUtil.isIgualACero(lineaOrigen.getImporteTotalPromocionesMenosIngreso())) {
				...
				BigDecimal precioSinImpuestos = sesion.getAplicacion().getTienda().getCliente().getCodpais().equals("SG")
				        ? bylSesionImpuestos.getPrecioSinImpuestos(lineaOrigen.getCodImpuesto(), precioSinPromocionesMenosIngreso, lineaOrigen.getCabecera().getCliente().getIdTratImpuestos())
				        : sesionImpuestos.getPrecioSinImpuestos(lineaOrigen.getCodImpuesto(), precioSinPromocionesMenosIngreso, lineaOrigen.getCabecera().getCliente().getIdTratImpuestos());
				...
				// igualmos los precios sin DTO a los precios con DTO, en devoluciones no hay
				// descuentos
				lineaOrigen.setPrecioTotalConDto(
				        lineaOrigen.getImporteTotalConDto().setScale(6, BigDecimal.ROUND_HALF_UP).divide(lineaOrigen.getCantidad().setScale(6, BigDecimal.ROUND_HALF_UP), BigDecimal.ROUND_HALF_UP));
		...
		if (promocionMenosIngreso) {
			// Añadimos a la ultima linea la diferencia entre el total de la cabecera del
			// ticket y el total de precio de las lineas
			if (!lineas.isEmpty()) {
				...
				BigDecimal precioSinImpuestos = sesion.getAplicacion().getTienda().getCliente().getCodpais().equals("SG")
				        ? bylSesionImpuestos.getPrecioSinImpuestos(ultimaLinea.getCodImpuesto(), ultimaLinea.getPrecioTotalSinDto(), idTratamientoImpuesto)
				        : sesionImpuestos.getPrecioSinImpuestos(ultimaLinea.getCodImpuesto(), ultimaLinea.getPrecioTotalSinDto(), idTratamientoImpuesto);
			...
			// Añadimos a la ultima linea la diferencia entre la suma de importes de las
			// lineas del ticket original y la suma de importes de las lineas del ticket de
			// devolución
	...
	public boolean comprobarConfigContador(String codDocumento) {
		...
		try {
			...
			// documentos.getDocumento(ticketPrincipal.getCabecera().getTipoDocumento());
	...
	protected boolean comprobarRangoActivo(TipoDocumentoBean documento, ByLConfigContadorBean confContador)
	        throws ConfigContadoresRangosConstraintViolationException, ConfigContadoresRangosException, ConfigContadoresParametrosException {
		boolean res = true;
		Map<String, String> parametrosContador = new HashMap<>();
	...
	public void notificarContadores() {
		...
		// if (contador != null && ticketPrincipal != null && ((ByLCabeceraTicket)
		// ticketPrincipal.getCabecera()).getConfigContadorRango() != null) {
		if (contador != null && ticketPrincipal != null) {
			...
			/*
			 * En el caso de que estemos en Colombia, se realizara la comprobacion para enviar por email la notificacion
			 */
			envioEmailNotificacionCO(rangoSiguienteAviso, valorUltimoContador);
	...
	public synchronized LineaTicket nuevaLineaArticulo(String codArticulo, String desglose1, String desglose2, BigDecimal cantidad, boolean pesarArticulo, Integer idLineaDocOrigen,
		...
		ByLLineaTicket linea = null;
		BigDecimal precio = null;

		String codBarras = null;
		// Comprobamos si es codigo de barras especial o normal y actualizamos
		// codigoArticulo y otras variables
		try {
		...
		try {
			linea = (ByLLineaTicket) lineasTicketServices.createLineaArticulo((TicketVenta) ticketPrincipal, codArticulo, desglose1, desglose2, cantidad, precio, createLinea());
			...
			if (esDevolucion && ticketOrigen != null && !esLineaDevolucionPositiva) {
				...
				if (lineaOrigen instanceof ByLLineaTicket) { /*
				                                              * Si es un ticket de Navision, no viene con el
				                                              * xsi:type=bylLineaTicket, por lo que fallaría al realizar
				                                              * el casting
				                                              */
					if (((ByLLineaTicket) lineaOrigen).getNotaInformativa() != null) {
						((ByLLineaTicket) linea).setNotaInformativa(((ByLLineaTicket) lineaOrigen).getNotaInformativa());
					}
				}
	...
	public void recuperarTicket(TicketAparcadoBean ticketAparcado) throws TicketsServiceException, PromocionesServiceException, DocumentoException, LineaTicketException {
		...
		// TODO-REE Parche para poder recuperar tickets de FASTPOS - Si es fastpos, no
		// cambiamos cliente
		...
		for (LineaTicket lineaRecuperada : lineas) {
			...
			// Parte personalizada para ByL
			if (nuevaLineaArticulo instanceof ByLLineaTicket) { /*
			                                                     * Si es un ticket de Navision, no viene con el
			                                                     * xsi:type=bylLineaTicket, por lo que fallaría al
			                                                     * realizar el casting
			                                                     */
				((ByLLineaTicket) nuevaLineaArticulo).setNotaInformativa(((ByLLineaTicket) lineaRecuperada).getNotaInformativa());
			}
			nuevaLineaArticulo.setVendedor(lineaRecuperada.getVendedor());

			/* Trazabilidad en linea de artículo */
			((ByLLineaTicket) nuevaLineaArticulo).setTrazabilidad(((ByLLineaTicket) lineaRecuperada).getTrazabilidad());
		}
	...
	private void tseFinishTransaction() {
		...
		try {
			...
			String peticionFinishTransaction = epsonTSEService.finishTransaction(EpsonTSEService.PROCESSTYPE_KASSENBELEG_V1, EpsonTSEService.TIPO_TRANSACCION_BELEG,
			        getTicket().getTotales().getTotal(), pagosEfectivo, pagosNoEfectivo, Integer.parseInt(eposOutput.getTransactionNumber()), false);
	...
	public ByLTicketManager getTicketAuxiliar() {
		return ticketAuxiliar;
	}

	public void setTicketAuxiliar(ByLTicketManager ticketAuxiliar) {
		this.ticketAuxiliar = ticketAuxiliar;
	}

	public ByLTicketVentaAbono recuperarTicketFidelizadoByL(String codTicket, String codTienda, String codCaja, Long idTipoDoc) throws ArticuloNoAptoException {
		...
		try {
			...
			// Por codigo desde central
			try {
				respuesta = obtenerTicketDevolucionCentralByLbyCodTicket(codCaja, codTienda, codTicket, idTipoDoc);
				if (respuesta.getTicket() != null) {
					xmlTicketOrigen = respuesta.getTicket().getBytes("UTF-8");
			...
			if (xmlTicketOrigen == null) {
				// Si null, obtenemos por codigo desde local
				TicketBean ticketA = ticketsService.consultarTicketAbonobyCodTicket(codTienda, codCaja, codTicket, idTipoDoc);
				if (ticketA != null) {
					xmlTicketOrigen = ticketA.getTicket();
				}
				else {
					throw new TicketsServiceException("No se ha encontrado ticket con codigo: " + codTicket);
				}
			}
	...
	public List<Class<?>> getTicketClasses(TipoDocumentoBean tipoDocumento) {
		...
		classes.add(SpringContext.getBean(ByLTicketVentaAbono.class).getClass());
	...
	public boolean comprobarTratamientoFiscalDev() {
		...
		/*
		 * Si estamos en USA no tendremos en cuenta la compatibilidad de los tratamientos fiscales
		 */
		if (sesion.getAplicacion().getTienda().getCliente().getCodpais().equals(ByLPagosController.CODIGO_PAIS_USA)) {
			return true;
		}
	...
	private void envioEmailNotificacionCO(Long rangoSiguienteAviso, Long valorUltimoContador) {
		if (sesion.getAplicacion().getTienda().getCliente().getCodpais().equals(COD_COLOMBIA)) {
			log.debug("envioEmailNotificacionCO()");
			/* BYL-159 Envío email finalización del contador */
			ConfigContadorRango contadorRango = ServicioConfigContadoresRangos.get().consultar(contador.getIdContador(), contador.getIdRangoUltimaPeticion());
			if (contadorRango != null) {

				if ((rangoSiguienteAviso != null && rangoSiguienteAviso <= valorUltimoContador)
				        || (contadorRango.getProximoAvisoFecha() != null && new Fecha().getDate().compareTo(contadorRango.getProximoAvisoFecha()) >= 0)) {

					try {
	                    Date hoy = new Fecha().getDate();
	                    Date proximo = contadorRango.getProximoAvisoFecha();
	                    String motivo = (rangoSiguienteAviso != null && rangoSiguienteAviso <= valorUltimoContador)
	                            ? "CONTADOR"
	                            : ((proximo != null && hoy.compareTo(proximo) >= 0) ? "FECHA" : "DESCONOCIDO");
	                    
	                    log.info("BYL- envioEmailNotificacionCO() - enviarCorreoRango()"
	                            + " - Motivo: " + motivo
	                            + " - valorUltimoContador: " + valorUltimoContador
	                            + " - rangoSiguienteAviso: " + rangoSiguienteAviso
	                            + " - fecha actual: " + hoy
	                            + " - proximoAviso: " + proximo
	                            + " - idContador: " + contador.getIdContador()
	                            + " - idRangoUltimaPeticion: " + contador.getIdRangoUltimaPeticion());

	                    
						ticketsService.enviarCorreoRango(ticketPrincipal);
					}
					catch (Exception e) {
						log.error("notificarContadores() - No se ha podido enviar el correo: " + e.getMessage(), e);
					}

				}

			}
			/* fin BYL-159 Envío email finalización del contador */
		}
	}
	
	// Merchant Account es un atributo de la configuración del PINPAD
	@SuppressWarnings({ "rawtypes", "unchecked" })
	protected void addMerchantAccountToTicketPayments(ITicket ticketPrincipal) {
		if (Dispositivos.getInstance().getTarjeta() instanceof ByLTefAdyenCloud) {
			ByLTefAdyenCloud pinpad = (ByLTefAdyenCloud) Dispositivos.getInstance().getTarjeta();
			String merchantAccount = pinpad.getConfiguracion().getParametrosConfiguracion().get(MERCHANT_ACCOUNT);

			// // Métodos de pago que tenga configurado el pinpad de AdyenCloud
			// String paymentsCode = pinpad.getConfiguracion().getParametrosConfiguracion().get("PAYMENTS");
			// String paymentsCode2 = pinpad.getConfiguracion().getParametrosConfiguracion().get("PAYMENTS2");
			// List<String> paymentsCodeList = Arrays.asList(paymentsCode, paymentsCode2);

			for (PagoTicket pago : (List<PagoTicket>) ticketPrincipal.getPagos()) {
				if (pinpad.isCodMedPagoAceptado(pago.getCodMedioPago()) && pago.getDatosRespuestaPagoTarjeta() != null) {
					pago.getDatosRespuestaPagoTarjeta().getAdicionales().put(MERCHANT_ACCOUNT, merchantAccount);
				}
			}
		}
